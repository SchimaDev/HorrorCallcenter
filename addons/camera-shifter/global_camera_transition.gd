...
@onready var global_camera_transition_2d: Camera2D = $GlobalCameraTransition2D
@onready var global_camera_transition_3d: Camera3D = $GlobalCameraTransition3D
...
var transitioning_2d := false
var transitioning_3d := false


func _ready():
	assert(global_camera_transition_2d is Camera2D, "The autoload GlobalCameraTransition needs a Camera2D in scene to allow the transitions")
	assert(global_camera_transition_3d is Camera3D, "The autoload GlobalCameraTransition needs a Camera3D in scene to allow the transitions")
...
func transition_to_requested_camera_2d(from: Camera2D, to: Camera2D, duration: float = transition_duration, record_transition: bool = true):
	...
	transitioning_2d = true
	
	global_camera_transition_2d.make_current()
	
	global_camera_transition_2d.offset = from.offset
	global_camera_transition_2d.global_transform = from.global_transform
	global_camera_transition_2d.anchor_mode = from.anchor_mode
	global_camera_transition_2d.zoom = from.zoom
	global_camera_transition_2d.process_callback = from.process_callback
	global_camera_transition_2d.limit_smoothed = from.limit_smoothed
	global_camera_transition_2d.position_smoothing_enabled = from.position_smoothing_enabled
	global_camera_transition_2d.rotation_smoothing_enabled = from.rotation_smoothing_enabled
	...
	transition_tween_2d.tween_property(global_camera_transition_2d, "global_transform", to.global_transform, duration).from(global_camera_transition_2d.global_transform)
	transition_tween_2d.tween_property(global_camera_transition_2d, "zoom", to.zoom, duration).from(global_camera_transition_2d.zoom)
	await transition_tween_2d.finished
	
	if record_transition:
		transition_steps_2d.append(TransitionStep2D.new(from, to, duration))
	
	transitioning_2d = false
	to.make_current()
...
func transition_to_requested_camera_3d(from: Camera3D, to: Camera3D, duration: float = transition_duration, record_transition: bool = true):
	...
	transitioning_3d = true
	
	global_camera_transition_3d.make_current()
	
	global_camera_transition_3d.projection = to.projection
	
	match to.projection:
		Camera3D.PROJECTION_ORTHOGONAL:
			global_camera_transition_3d.size = from.size
		Camera3D.PROJECTION_FRUSTUM:
			global_camera_transition_3d.frustum_offset = from.frustum_offset
		Camera3D.PROJECTION_PERSPECTIVE:
			global_camera_transition_3d.fov = from.fov
		
	global_camera_transition_3d.far = from.far
	global_camera_transition_3d.near = from.near
	global_camera_transition_3d.keep_aspect = from.keep_aspect
	global_camera_transition_3d.fov = from.fov
	global_camera_transition_3d.cull_mask = from.cull_mask
	global_camera_transition_3d.global_transform = from.global_transform
	
	global_camera_transition_3d.environment = from.environment
	global_camera_transition_3d.attributes = from.attributes
	global_camera_transition_3d.doppler_tracking = from.doppler_tracking
	
	transition_tween_3d = create_tween()
	transition_tween_3d.set_parallel(true).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_CUBIC)
	transition_tween_3d.tween_property(global_camera_transition_3d, "global_transform", to.global_transform, duration).from(global_camera_transition_3d.global_transform)
	
	match to.projection:
		Camera3D.PROJECTION_ORTHOGONAL:
			transition_tween_3d.tween_property(global_camera_transition_3d, "size", to.size, duration).from(global_camera_transition_3d.size)
		Camera3D.PROJECTION_FRUSTUM:
			transition_tween_3d.tween_property(global_camera_transition_3d, "frustum_offset", to.frustum_offset, duration).from(global_camera_transition_3d.frustum_offset)
		Camera3D.PROJECTION_PERSPECTIVE:
			transition_tween_3d.tween_property(global_camera_transition_3d, "fov", to.fov, duration).from(global_camera_transition_3d.fov)
...
func is_transitioning_2d() -> bool:
	return transitioning_2d or (transition_tween_2d and transition_tween_2d.is_running())
	

func is_transitioning_3d() -> bool:
	return transitioning_3d or (transition_tween_3d and transition_tween_3d.is_running())
